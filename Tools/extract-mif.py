...
(magic,) = struct.unpack_from('<6s', data,
                              offset)  # '<6s'  =  '<' little-endian, 's' is type char, '6s' Array of 6 chars; get
# first entry of the returned tuple
...
(footer_offset,) = struct.unpack_from('<I', data, offset)
# unpack '<' little-endian, 'I' unsigned-int; get first entry of the
# returned tuple
...
for image_id in range(0, int(num_files)):
    (width, height, bytes_per_pixel, zsize) = struct.unpack_from('<IIII', data, offset)
    # put unsigned-int,unsigned-int,
    # unsigned-int,unsigned-int in variables width, height, bytes_per_pixel, zsize
    print("Image %d - offset: %d / width: %d px / height: %d px / bpp: %d / zsize: %d bytes" % (
        image_id, offset, width, height, bytes_per_pixel, zsize))

    image_zlib = data[16 + offset:16 + offset + zsize]
    # 16 because of unsigned-int,unsigned-int,unsigned-int, unsigned-int from above

    # to ensure it's extracted in the right way, since this archive can have both 1bpp and 4bpp, sometimes even 8bpp
    # png's inside.
