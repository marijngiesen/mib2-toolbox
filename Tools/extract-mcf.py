...
# '<4s'  =  '<' little-endian, 's' is type char, '6s' Array of 6 chars; get
...
for image_id in range(0, int(num_files)):
    (file_type, file_id, file_offset, file_size) = struct.unpack_from('<4sIII', data, offset)
    # file_size = meta information (size of 40) + zsize
...
for image_id in range(0, int(num_files)):
    ...
    (type, file_id, always_8, zsize, max_pixel_count, always_1, unknown_16, width, height, image_mode,
     always__1) = struct.unpack_from('<4sIIIIIIhhhh', data, offset)
    ...
    try:
        ...
        print("error on %s;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d" % (
            type, file_id, always_8, zsize, max_pixel_count, always_1, unknown_16, width, height, image_mode,
            always__1))
