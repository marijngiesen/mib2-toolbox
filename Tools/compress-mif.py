...
(magic,) = struct.unpack_from('<6s', data, offset)
# '<6s'  =  '<' little-endian, 's' is type char, '6s' Array of 6 chars; get
...
(original_footer_offset,) = struct.unpack_from('<I', data,
                                               offset)  # unpack '<' little-endian, 'I' unsigned-int; get first entry
...
for image_id in range(0, int(num_files)):
    ...
    image_id, footer_offset, width, height, bytes_per_pixel, zsize))
...
struct_data = struct_magic + struct_footer_offset + struct_data + struct_num_files + struct_offset_table + struct_uid\
              + struct_meta_name_len + struct_meta_name + struct_meta_author_len + struct_meta_author
