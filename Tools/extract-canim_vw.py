...
while cmd_code == 0x11:
    print('generating img_%d from offset %d \t  size: %d x %d \t bpp: %d \t size: %d bytes' % (
    img_num, cmdblock_len + data_start + 32, img_width, img_height, bytes_per_pixel,
    img_width * img_height * bytes_per_pixel))

    if bytes_per_pixel == 0x3:
        im = Image.frombuffer('RGB', (img_width, img_height), data[0x20 + cmdblock_len + data_start:], 'raw', 'RGB', 0,
                              1)
    if bytes_per_pixel == 0x4:
        im = Image.frombuffer('RGBA', (img_width, img_height), data[0x20 + cmdblock_len + data_start:], 'raw', 'RGBA',
                              0, 1)
