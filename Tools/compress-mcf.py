...
for image_id in range(0, int(num_files)):
    (original_type, original_file_id, original_always_8, original_zsize, original_max_pixel_count, original_always_1,
     original_unknown_hash1, original_width, original_height, original_image_mode,
     original_always__1) = struct.unpack_from('<4sIIIIIIhhhh', data, offset_original)
    ...
    print("%s;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d" % (
        type, file_id, always_8, zsize, max_pixel_count, always_1, unknown_hash1, width, height, image_mode, always__1,
        unknown_hash2))

    # print ("%s;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d"%(original_type, original_file_id, original_always_8,
    # original_zsize, original_max_pixel_count, original_always_1, original_unknown_hash1, original_width,
    # original_height, original_image_mode, original_always__1, original_unknown_hash2))

    struct_data = struct_data + struct.pack('<4sIIIIIIhhhh', type, file_id, always_8, zsize, max_pixel_count, always_1,
                                            unknown_hash1, width, height, image_mode,
                                            always__1) + image_zlib + struct.pack('<I', original_unknown_hash2)
    struct_toc = struct_toc + struct.pack('<4sIII', type, file_id, offset_new,
